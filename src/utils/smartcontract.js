...
export const loadWeb3 = async () => {
  ...
  if (provider) {
    ...
    if (netId !== 97) {
      toast.error("Your Wallet network is not supported yet, please select BSC Testnet", { delay: 1000 });
      return false;
    }
    ...
    const handleAddOrder = async () => {
      ...
      try {
        marketPlaceContract.methods
          .addOrder(parseInt(nftId), PAYMENT_TOKEN, Web3.utils.toWei(price.toString(), 'ether'))
          .send({ from: sellerAddress })
          .once('transactionHash', setHash)
          .once("receipt", setRecept)
          .on('error', setError);
  ...
  useEffect(() => {
    const approveTokenContract = async () => {
      ...
      try {
        const contractToken = await getTokenContract(
          paymentTokenAbi, PAYMENT_TOKEN
        );
        contractToken.methods.approve(
          NFT_MARKET_ADDR,
          MAX_NUM,
        )
    ...
    const checkTokenApproved = async () => {
      ...
      try {
        ...
        const balance = await contractToken.methods.balanceOf(addressBuyer).call();
